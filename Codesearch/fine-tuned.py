...
class CodeSearchModel(nn.Module):
    def __init__(self, base_model, hidden_dim=768, projection_dim=256):
        super().__init__()
        self.encoder = base_model
        # Projection head to map from hidden_dim to projection_dim
    ...
    def forward(self, code_input_ids, code_attention_mask, query_input_ids, query_attention_mask):
        ...
        code_emb = F.normalize(projected_emb[:batch_size], p=2, dim=1)
        query_emb = F.normalize(projected_emb[batch_size:], p=2, dim=1)
...
def get_embeddings(text_list, model, tokenizer, batch_size=16, max_len=256):
    ...
    for i in tqdm(range(0, len(text_list), batch_size), desc="Embedding"):
        ...
        with torch.no_grad():
            cls_emb = model.encoder(**inputs).last_hidden_state[:, 0, :]
            # Apply projection head
            projected_emb = model.projection(cls_emb)
            embs.append(F.normalize(projected_emb, p=2, dim=1))
...
model = CodeSearchModel(base_model, hidden_dim=768, projection_dim=256)
...
def get_embeddings(text_list, model, tokenizer, batch_size=16, max_len=256):
    ...
    for i in tqdm(range(0, len(text_list), batch_size), desc="Embedding"):
        ...
        with torch.no_grad():
            ...
            # Apply projection head
            projected_emb = model.projection(cls_emb)
            emb = F.normalize(projected_emb, p=2, dim=1)
            all_embs.append(emb)
